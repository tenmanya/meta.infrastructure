...
resource "github_repository_environment" "bananas-rails-production" {
  repository  = github_repository.bananas-rails.name
  environment = "production"

  reviewers {
    users = [
      data.github_user.gkameari.id,
      data.github_user.jtsaito.id,
    ]
  }
...
resource "github_actions_environment_variable" "bananas-rails-production-aws-iam-role-terraform-apply" {
  repository  = github_repository.bananas-rails.name
  environment = github_repository_environment.bananas-rails-production.environment

  variable_name = "AWS_IAM_ROLE_TERRAFORM_APPLY"
  value         = aws_iam_role.terraform-apply.arn
}

resource "github_actions_environment_variable" "bananas-rails-production-environment" {
  repository  = github_repository.bananas-rails.name
  environment = github_repository_environment.bananas-rails-production.environment
...
resource "github_actions_environment_variable" "bananas-rails-production-aws-region-terraform-apply" {
  repository  = github_repository.bananas-rails.name
  environment = github_repository_environment.bananas-rails-production.environment
...
resource "github_actions_environment_variable" "bananas-rails-production-tf-backend" {
  for_each = {
...
resource "github_actions_environment_variable" "bananas-rails-productin-tf-backend-key" {
  repository  = github_repository.bananas-rails.name
  environment = github_repository_environment.bananas-rails-production.environment

  variable_name = "TF_BACKEND_KEY"
  value         = "${github_repository.bananas-rails.name}/production/terraform.tfstate"
}
